...
export function DirectoryOverview({
  ...
  return (
    <Card className="relative overflow-hidden">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,color-mix(in_oklab,var(--accent-primary)_16%,transparent),transparent_42%),linear-gradient(135deg,color-mix(in_oklab,var(--bg-elevated)_75%,transparent),transparent)]" />
      <div className="relative space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl space-y-3">
            ...
            {featuredLineages.length ? (
              <div className="flex flex-wrap gap-2 pt-1">
                {featuredLineages.slice(0, 4).map((lineage) => (
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[color-mix(in_oklab,var(--bg-surface)_86%,transparent)] p-4 backdrop-blur"
            >
